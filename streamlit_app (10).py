...
# ══════════════════════════════════════════════════════════════════
# MAIN LAYOUT — ימין: קלט | שמאל: פלט (מתעדכן בזמן אמת)
# ══════════════════════════════════════════════════════════════════
col_input, col_output = st.columns([1.1, 0.9], gap="medium")

with col_input:
    run_clicked = render_mix_builder(df_active, all_funds)

with col_output:
    # ── שלב 1: טבלת מצב קיים (מיד כשנועלים קרנות) ──
    ...
    # ── שלב 2: החלופה הטובה ביותר (אחרי חישוב) ──
...
# ── RESULTS — מוצגות ברוחב מלא מתחת לשתי העמודות ──────────────────
# הסדר ההגיוני: קלט (ימין) | פלט מיידי (שמאל) → תוצאות מפורטות (רוחב מלא)
if res and recs:
    # בנה רשימת חלופות
    rows_list = []
    ...
    # מפריד ויזואלי
    st.markdown("<hr style='border:none;border-top:1px solid #E5EAF2;margin:8px 0 16px'>",
                unsafe_allow_html=True)

    # Results strip — רוחב מלא
