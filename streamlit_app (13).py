...
def render_current_state(df_active, all_funds_list):
    ...
    st.markdown("""
<div class='card'>
<div class='card-title' style='font-size:16px'>📂 מצב קיים וסימולציות לשינוי</div>
...
def render_target_mix(df_active, all_funds_list):
    ...
    st.markdown(f"<div class='card'>\n<div class='card-title' style='font-size:16px'>🎯 יעד לתמהיל{_esc(title_suffix)}</div>", unsafe_allow_html=True)
...
<div style='overflow-x:auto;border:1px solid #E5EAF2;border-radius:10px;overflow:hidden'>
  <table class='res-tbl'>
    ...
    <tbody>{rows_h}</tbody>
        ...
        bar_df = comp_df[["קרן", bar_metric]].dropna().sort_values(bar_metric, ascending=True)
        if not bar_df.empty:
            colors = ["#3A7AFE"] * len(bar_df)
            if len(bar_df) > 1:
                colors[-1] = "#15A46E"; colors[0] = "#EF4444"
            unit = "%" if "%" in bar_metric else ""
            # גרף אופקי — קריא גם עם שמות ארוכים
            fig = go.Figure(go.Bar(
                y=bar_df["קרן"], x=bar_df[bar_metric],
                orientation="h",
                marker_color=colors,
                text=bar_df[bar_metric].apply(lambda v: f"{v:.1f}{unit}"),
                textposition="outside",
            ))
            # חשב גובה דינמי לפי מספר הקרנות
            bar_height = max(280, len(bar_df) * 36)
            fig.update_layout(
                height=bar_height,
                margin=dict(t=15, b=15, l=5, r=80),
