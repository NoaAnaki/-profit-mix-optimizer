...
# ─────────────────────────────────────────────
# CSS – Profit Financial Group Design System v4.0
# ─────────────────────────────────────────────
st.markdown("""
<style>
/* ══ DESIGN TOKENS ══════════════════════════════════════════════════ */
:root {
  --profit-black:   #1A1A1A;
  --profit-crimson: #A31235;
  --profit-crimson-dark: #8B0F2B;
  --navy:           #1F3A5F;
  --blue:           #3A7AFE;
  --blue-light:     #EFF6FF;
  --blue-border:    #C7D7FF;
...
div[data-testid="stButton"] > button[kind="primary"] {
  background: var(--profit-crimson) !important;
  color: #fff !important;
  font-weight: 800 !important;
...
div[data-testid="stButton"] > button[kind="primary"]:hover {
  background: var(--profit-crimson-dark) !important;
  box-shadow: 0 4px 14px rgba(163,18,53,0.4) !important;
...
/* Tabs (product selector uses st.radio → .nav-bar) */
...
.pmo-logo-box {
  ...
  background: var(--profit-crimson);
  display: flex; align-items: center;
  justify-content: center; font-size: 20px; flex-shrink: 0;
  box-shadow: 0 2px 8px rgba(163,18,53,0.4);
}
.pmo-title {
  font-size: 20px; font-weight: 800; margin: 0;
  letter-spacing: -0.3px; color: #fff;
}
.pmo-sub   { font-size: 11px; color: #93b4e0; margin: 2px 0 0; }
.pmo-kpis  { display: flex; gap: 16px; }
.pmo-kpi   { text-align: center; }
.pmo-kpi-val {
  font-size: 17px; font-weight: 800; color: #7dd3fc;
  display: block; line-height: 1.1;
}
.pmo-kpi-lbl { font-size: 10px; color: #93b4e0; white-space: nowrap; }

/* ══ PRODUCT TABS (via Streamlit radio) ═════════════════════════════ */
...
.nav-bar div[role="radiogroup"] label:hover {
  ...
  color: var(--navy) !important;
  transform: translateY(-1px) !important;
  box-shadow: 0 3px 10px rgba(58,122,254,0.2) !important;
}
...
.card-sub { font-size: 11px; color: var(--gray-text); margin-bottom: 8px; }
...
.mix-total .t-ok   { color: var(--green);  font-weight: 800; }
.mix-total .t-warn { color: var(--amber);  font-weight: 800; }
.mix-total .t-err  { color: var(--red);    font-weight: 800; }

/* ══ BEST RESULT CARD (dark navy) ═══════════════════════════════════ */
.br-score {
  font-size: 52px; font-weight: 900; color: #7dd3fc;
  line-height: 1; margin: 0;
}
.br-score-lbl { font-size: 11px; color: #93b4e0; margin-top: 2px; }
.br-managers  { font-size: 12px; color: #e2eeff; margin: 8px 0 4px; }
.br-tracks    { font-size: 11px; color: #93b4e0; margin-bottom: 10px; }
...
.br-chip {
  ...
  background: #2a4a70; color: #bfdbfe; border: 1px solid #3a5f8a;
}
...
.rc-kpi-label { font-size: 10px; color: var(--gray-text); font-weight: 600; }
.rc-kpi-val   { font-size: 15px; font-weight: 800; color: var(--navy); }
.rc-kpi-delta { font-size: 10px; font-weight: 700; }
.rc-kpi-delta.up   { color: var(--green); }
.rc-kpi-delta.down { color: var(--red); }
.rc-kpi-delta.flat { color: var(--gray-text); }
...
.res-tbl th {
  background: var(--navy); color: #F0F4FF;
  padding: 8px 10px; text-align: right;
  white-space: nowrap; font-weight: 700; font-size: 11.5px;
}
.res-tbl th:first-child { border-radius: 0 8px 0 0; }
.res-tbl th:last-child  { border-radius: 8px 0 0 0; }
.res-tbl td {
  padding: 8px 10px;
...
.res-tbl tr.sel-row td { background: var(--blue-light) !important; }
.res-tbl tr:hover td   { background: #F9FAFB; }
.res-tbl td.num  { text-align: center; font-variant-numeric: tabular-nums; }
...
.rc:hover { box-shadow: 0 4px 16px rgba(31,58,95,0.1); }
.rc-primary { border: 2px solid var(--profit-crimson); }
.rc-service { border: 2px solid var(--green); }
.rc-head {
  display: flex; justify-content: space-between;
  align-items: flex-start; margin-bottom: 8px;
}
.rc-title { font-size: 14px; font-weight: 800; color: var(--navy); }
.rc-badge {
  padding: 3px 10px; border-radius: 999px;
  font-size: 11px; font-weight: 700;
}
.rc-badge-blue   { background: var(--blue-light); color: var(--navy); border: 1px solid var(--blue-border); }
.rc-badge-green  { background: #dcfce7; color: #166534; border: 1px solid #a7f3d0; }
.rc-badge-amber  { background: #fef3c7; color: #92400e; border: 1px solid #fde68a; }
.rc-kpis { display: flex; gap: 6px; flex-wrap: wrap; margin: 10px 0; }
.rc-funds { margin: 8px 0; }
.rc-fund-row {
  display: flex; align-items: center; gap: 8px;
  margin-bottom: 5px; font-size: 12px;
}
.rc-fund-pct { font-weight: 800; color: var(--navy); min-width: 36px; }
.rc-fund-bar-wrap {
  flex: 1; height: 6px; background: #e5eaf2;
  border-radius: 3px; overflow: hidden;
}
.rc-fund-bar { height: 6px; border-radius: 3px; }
.rc-fund-name { font-size: 11px; color: var(--gray-text); }
.rc-ai {
  background: #f8faff; border: 1px solid var(--blue-border);
  border-radius: 8px; padding: 8px 12px; margin-top: 8px;
}
.rc-ai-label { font-size: 10px; color: var(--blue); font-weight: 700; margin-bottom: 4px; }
.rc-ai-text  { font-size: 12px; color: #374151; line-height: 1.5; }
.rc-ai-spinner { font-size: 11px; color: var(--gray-text); font-style: italic; }
.rc-delta { margin-top: 10px; }
.rc-delta-title { font-size: 11px; font-weight: 700; color: var(--gray-text); margin-bottom: 6px; }
.rc-delta-grid { display: flex; gap: 8px; flex-wrap: wrap; }
.rc-delta-item {
  background: var(--bg); border-radius: 6px;
  padding: 5px 10px; font-size: 11px; text-align: center;
  border: 1px solid var(--gray-border); min-width: 70px;
}
.rc-delta-item-label { color: var(--gray-text); font-weight: 600; display: block; margin-bottom: 2px; }
.rc-delta-item-vals  { display: flex; align-items: center; gap: 4px; justify-content: center; }
.rc-delta-item-cur   { color: var(--gray-text); }
.rc-delta-item-new   { color: var(--navy); font-weight: 800; }
.rc-delta-item-arrow { font-size: 10px; }
.rc-delta-item-arrow.up   { color: var(--green); }
.rc-delta-item-arrow.down { color: var(--red); }
.rc-delta-item-arrow.flat { color: var(--gray-text); }
...
.pw-wrap  {
  ...
  box-shadow: 0 8px 32px rgba(31,58,95,0.12);
}
.pw-title { font-size: 22px; font-weight: 800; color: var(--navy); margin-bottom: 4px; }
.pw-sub   { font-size: 12px; color: var(--gray-text); margin-bottom: 14px; }
.pw-warn  {
  font-size: 11px; color: var(--amber);
  background: #FEF3C7; border-radius: 6px;
  padding: 4px 9px; margin-top: 7px;
}
...
.score-tip {
  ...
  font-size: 11.5px; color: #78350F; margin: 5px 0;
}
...
@media (max-width: 768px) {
  ...
  .rc-kpis        { gap: 4px; }
  .rc-delta-grid  { gap: 4px; }
  .nav-bar div[role="radiogroup"] { gap: 4px !important; }
  .nav-bar div[role="radiogroup"] label { padding: 6px 12px !important; font-size: 12px !important; }
}

/* ══ DARK MODE ══════════════════════════════════════════════════════ */
@media (prefers-color-scheme: dark) {
  html, body, [class*="css"] { background: #0d1117 !important; }
  .card { background: #161b22 !important; border-color: #30363d !important; }
  .card-title { color: #f0f6ff !important; }
  .pmo-header { background: #0f2440 !important; }
  .res-tbl td { border-color: #1e293b !important; color: #cbd5e1 !important; }
  .res-tbl th { background: #1e3a8a !important; }
  .res-strip, .mix-total { background: #0f1e3d !important; border-color: #1e3a5f !important; color: #cbd5e1 !important; }
  .br-chip { background: #0f1e3d !important; border-color: #1e3a5f !important; color: #93c5fd !important; }
  .score-tip { background: #451a03 !important; border-color: #92400e !important; color: #fde68a !important; }
  .rc { background: #161b22 !important; border-color: #30363d !important; }
  .rc-ai { background: #0f1e3d !important; border-color: #1e3a5f !important; }
  .pw-wrap { background: #161b22 !important; border-color: #30363d !important; }
  .pw-title { color: #f0f6ff !important; }
  .rc-delta-item { background: #1e293b !important; border-color: #2d3748 !important; }
  .nav-bar div[role="radiogroup"] label { background: #161b22 !important; border-color: #30363d !important; color: #cbd5e1 !important; }
  .nav-bar div[role="radiogroup"] label:has(input:checked) { background: #1e3a8a !important; color: #fff !important; border-color: #3a7afe !important; }
  div[data-testid="stButton"] > button[kind="secondary"] { background: #161b22 !important; color: #cbd5e1 !important; border-color: #30363d !important; }
}
</style>
...
def render_mix_builder(df_active, all_funds_list):
    ...
    with st.expander("⚙️ הגדרות מתקדמות והצגת מצב קיים", expanded=False):

        # ══ 1. נעילת קרנות — ראשון בסדר ══════════════════════════════════
        st.markdown("#### 🔒 נעילת קרנות מצב קיים")
        st.caption("בחר קרנות שכבר קיימות אצל הלקוח — מכל מוצר שצריך. המערכת תחשב השלמה.")
        ...
        for prod_name in all_products_funds:
            # get fund list for this product from df_long if it's the active product, else skip
            if prod_name == st.session_state.get("product_type", "קרנות השתלמות"):
                fund_list_for_prod = sorted(df_long["fund"].unique().tolist())
            else:
                # Use cached df for other products (lightweight — already cached)
                _pid = all_products_funds[prod_name]
                _df_other, _, _ = load_funds_long(_pid, SERVICE_GSHEET_ID, prod_name)
                fund_list_for_prod = sorted(_df_other["fund"].unique().tolist()) if not _df_other.empty else []

            if not fund_list_for_prod:
                continue

            cur_sel = [f for f in locked_funds_state.get(prod_name, []) if f in fund_list_for_prod]
            new_sel = st.multiselect(
                prod_name,
                options=fund_list_for_prod,
                default=cur_sel,
                key=f"lock_multi_{prod_name}",
                placeholder="הקלד שם קרן לחיפוש..."
            )
        ...
        all_locked_pairs = [
            (prod, fund)
            for prod, funds in locked_funds_state.items()
            for fund in funds
        ]
        ...
        if all_locked_pairs:
            ...
            for prod, fund in all_locked_pairs:
                ...
                amt = st.number_input(
                    f"{fund}  |  {prod}",
                    min_value=0.0, value=cur_amt, step=10000.0, format="%.0f",
        ...
        with col_d1:
            distrib_amt = st.number_input(
                ...
                min_value=0.0,
                value=float(st.session_state.get("distrib_amount", 0.0)),
                step=10000.0, format="%.0f", key="adv_distrib_amt"
            )
...
padding:10px 14px;margin-top:4px;text-align:center'>
  ...
  <div style='font-size:10px;color:#9CA3AF'>
    נעול: ₪{total_locked_sum:,.0f} &nbsp;|&nbsp; לחלוקה: ₪{distrib_amt:,.0f}
        ...
        # backwards-compat לאופטימייזר הקיים
        active_prod = st.session_state.get("product_type", "קרנות השתלמות")
        active_locked_funds = [f for p, f in all_locked_pairs if p == active_prod]
        if active_locked_funds:
            first_fund = active_locked_funds[0]
            first_key  = f"{active_prod}::{first_fund}"
            st.session_state["locked_fund"]   = first_fund
        ...
        # _is_default_unchecked / _DEFAULT_UNCHECKED_PATTERNS defined at module level
    ...
    # ── חישוב נתוני מצב קיים ───────────────────────────────────────────────────
    locked_amounts_state = st.session_state.get("locked_amounts_map", {})
    locked_funds_state   = st.session_state.get("locked_funds_multi", {})
    all_locked_pairs_slider = [
        (prod, fund)
        for prod, funds in locked_funds_state.items()
        for fund in funds
    ]
    distrib_amt      = float(st.session_state.get("distrib_amount", 0.0))
    total_locked_amt = sum(
        float(locked_amounts_state.get(f"{p}::{f}", 0.0))
        for p, f in all_locked_pairs_slider
    )
    grand_total = total_locked_amt + distrib_amt
    w_locked = (total_locked_amt / grand_total) if grand_total > 0 else 0.0
    w_new    = (distrib_amt      / grand_total) if grand_total > 0 else 1.0

    def _wlocked_param(param_key):
        total_w, total_v = 0.0, 0.0
        for prod, fund in all_locked_pairs_slider:
            key_id = f"{prod}::{fund}"
            amt = float(locked_amounts_state.get(key_id, 0.0))
            if amt <= 0: continue
            match = df_long[df_long["fund"].str.strip() == fund.strip()]
    ...
    if all_locked_pairs_slider:
        ...
        for prod, fund in all_locked_pairs_slider:
            ...
            def _fv(k):
                ...
                v = _to_float(match.iloc[0].get(k, None))
                return f"{v:.1f}%" if v is not None and not (isinstance(v,float) and math.isnan(v)) else "—"
            rows_h += f"""<tr>
  <td style='padding:6px 10px;font-weight:700;font-size:12px'>{_esc(fund)}</td>
  <td style='padding:6px 10px;color:#6B7280;font-size:11px'>{_esc(prod)}</td>
  <td style='padding:6px 10px;text-align:center'>₪{amt:,.0f}</td>
  <td style='padding:6px 10px;text-align:center;font-weight:700;color:#1F3A5F'>{w_pct:.1f}%</td>
  <td style='padding:6px 10px;text-align:center'>{_fv("stocks")}</td>
  <td style='padding:6px 10px;text-align:center'>{_fv("foreign")}</td>
  <td style='padding:6px 10px;text-align:center'>{_fv("fx")}</td>
  <td style='padding:6px 10px;text-align:center'>{_fv("illiquid")}</td>
</tr>"""
        def _sv(k):
  ...
  <td style='padding:6px 10px;text-align:center'>₪{total_locked_amt:,.0f}</td>
...
<div class='card' style='margin-bottom:10px'>
  <div class='card-title'>📋 מצב קיים — קרנות נעולות</div>
  <div style='overflow-x:auto'>
  <table style='width:100%;border-collapse:collapse;font-size:12.5px;direction:rtl'>
    <thead><tr style='background:#1F3A5F;color:#F0F4FF'>
      <th style='padding:7px 10px;text-align:right'>קרן</th>
      <th style='padding:7px 10px;text-align:right'>מוצר</th>
      <th style='padding:7px 10px;text-align:center'>סכום</th>
      <th style='padding:7px 10px;text-align:center'>משקל בתיק</th>
      <th style='padding:7px 10px;text-align:center'>מניות</th>
      <th style='padding:7px 10px;text-align:center'>חו"ל</th>
  ...
  <div style='font-size:11px;color:#6B7280;margin-top:6px'>
    ...
    # שמור משקלים לשימוש באופטימייזר ובתוצאות
    ...
    # ── תמהיל card ──
    title_suffix = " — על כלל התיק" if all_locked_pairs_slider else ""
    st.markdown(f"<div class='card'>\n<div class='card-title' style='font-size:16px'>⚙️ יעד תמהיל{_esc(title_suffix)}</div>", unsafe_allow_html=True)
    if all_locked_pairs_slider and grand_total > 0:
        st.markdown(
...
<p style='font-size:10px;color:#9CA3AF;margin:2px 0 6px'>
  לחץ על שם החלופה לפרטים וסטטיסטיקה &nbsp;|&nbsp; * שארפ חסר — לא כל המסלולים פרסמו שארפ &nbsp;|&nbsp; * שירות — ציון לא נמצא לגוף{portfolio_note}
</p>
""", unsafe_allow_html=True)
