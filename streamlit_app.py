...
.block-container { padding: 1.5rem 2rem 3rem !important; max-width: 98vw !important; }
...
# ── JS: פסיקים אוטומטיים בשדות מספר ──────────────────────────────────────────
st.markdown("""
<script>
(function() {
  function formatWithCommas(val) {
    ...
    if (!num) return '';
    return parseInt(num, 10).toLocaleString('he-IL');
  }
  function attachFormatters() {
    ...
    inputs.forEach(function(inp) {
      ...
      inp.addEventListener('input', function() {
        ...
        var formatted = formatWithCommas(inp.value);
  ...
  var observer = new MutationObserver(function() { attachFormatters(); });
...
""", unsafe_allow_html=True)
    ...
    # כפתור רענון נתונים — מנקה את ה-cache ומוריד מחדש מ-Sheets
...
""", unsafe_allow_html=True)
    ...
    if all_locked_pairs:
        ...
        for prod, fund in all_locked_pairs:
            ...
            cur_display = f"{cur_amt:,.0f}" if cur_amt > 0 else ""
            raw = st.text_input(
                f"{fund}  |  {prod}", value=cur_display,
                placeholder="הזן סכום בש״ח...", key=f"lock_amt_{key_id}"
            )
            try:
                amt = float(raw.replace(",","").replace(" ","").replace("₪","")) if raw.strip() else 0.0
            except ValueError:
                amt = cur_amt
    ...
    with col_d1:
        ...
        distrib_display = f"{cur_distrib:,.0f}" if cur_distrib > 0 else ""
        raw_distrib = st.text_input(
            "💰 סכום לחלוקה לקרנות נוספות (₪)",
            value=distrib_display, placeholder="הזן סכום בש״ח...", key="adv_distrib_amt"
        )
        try:
            distrib_amt = float(raw_distrib.replace(",","").replace(" ","").replace("₪","")) if raw_distrib.strip() else 0.0
        except ValueError:
            distrib_amt = cur_distrib
  ...
  <table class='res-tbl'>
    ...
    <tbody>{rows_h}</tbody>
        ...
        if not bar_df.empty:
            ...
            # שמות קצרים לציר X — חותך אחרי 18 תווים
            short_names = [n[:18] + "…" if len(n) > 18 else n for n in bar_df["קרן"]]
            fig = go.Figure(go.Bar(
                x=short_names,
                y=bar_df[bar_metric],
                marker_color=colors,
                ...
                textposition="outside",
                customdata=bar_df["קרן"],  # שם מלא ב-hover
...
# קלט ימין | פלט (טבלת מצב קיים) שמאל
# ══════════════════════════════════════════════════════════════════
st.markdown("<div class='section-title'>📂 שלב 2 — מצב קיים וסימולציות לשינוי</div>", unsafe_allow_html=True)
...
# קלט ימין | פלט (חלופה הטובה) שמאל
# ══════════════════════════════════════════════════════════════════
st.markdown("<hr style='border:none;border-top:2px solid #E5EAF2;margin:20px 0 14px'>", unsafe_allow_html=True)
st.markdown("<div class='section-title'>🎯 שלב 3 — יעד לתמהיל</div>", unsafe_allow_html=True)
