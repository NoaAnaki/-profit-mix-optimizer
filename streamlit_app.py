    ...
    with st.expander("⚙️ הגדרות מתקדמות והצגת מצב קיים", expanded=False):
        ...
        if all_locked_pairs:
            ...
            for prod, fund in all_locked_pairs:
                ...
                # text_input עם פסיקים + ריק בהתחלה
...
padding:10px 14px;margin-top:4px;text-align:center'>
  ...
  <div style='font-size:10px;color:#9CA3AF'>
    ...
    def _wlocked_param(param_key):
        ...
        for prod, fund in all_locked_pairs_slider:
            ...
            if amt <= 0: continue
            # חיפוש ב-df_all_locked שמכיל את כל המוצרים הנעולים
  ...
  <td style='padding:6px 10px;text-align:center;color:#1F3A5F'>{_sv("illiquid")}</td>
        ...
        # שמור את ה-HTML של הטבלה ב-session_state להצגה בעמודה הנכונה
  ...
  <div style='font-size:11px;color:#6B7280;margin-top:6px'>
    ...
    for key, label, default_on in params:
        ...
        with c2:
            ...
            if cur_locked is not None and all_locked_pairs_slider:
                # מציג: יעד + מצב קיים מתחתיו
                st.markdown(
...
# ══════════════════════════════════════════════════════════════════
# MAIN LAYOUT — ימין: קלט | שמאל: טבלת מצב קיים + חלופה טובה
# ══════════════════════════════════════════════════════════════════
...
with col_right:
    # טבלת מצב קיים — מוצגת מיד כשנועלים קרנות
    ...
    # החלופה הטובה ביותר — מוצגת אחרי חישוב
