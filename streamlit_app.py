...
""", unsafe_allow_html=True)
    ...
    if all_locked_pairs:
        ...
        for prod, fund in all_locked_pairs:
            ...
            raw = st.text_input(
                f"{fund}  |  {prod}", value=_fmt_ils(cur_amt),
                placeholder="לדוגמה: 300,000", key=f"lock_amt_{key_id}"
            )
            amt = _parse_amount(raw, cur_amt)
    ...
    with col_d1:
        ...
        raw_distrib = st.text_input(
            ...
            value=_fmt_ils(cur_distrib), placeholder="לדוגמה: 500,000", key="adv_distrib_amt"
        )
        distrib_amt = _parse_amount(raw_distrib, cur_distrib)
  ...
  <table class='res-tbl'>
    ...
    <tbody>{rows_h}</tbody>
        ...
        if not bar_df.empty:
            ...
            # שמות קצרים לציר X — חותך אחרי 15 תווים
            short_names = [n[:15] + "…" if len(n) > 15 else n for n in bar_df["קרן"]]
            fig = go.Figure(go.Bar(
                x=short_names,
                y=bar_df[bar_metric],
                marker_color=colors,
            ...
            fig.update_layout(
                height=360,
                margin=dict(t=30, b=150, l=10, r=10),
                xaxis=dict(
                    tickangle=-40,
